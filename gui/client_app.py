    ...
    def login():
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                global current_user, access_token, refresh_token
                ...
                app.after(100, lambda: show_frame("dashboard"))
            else:
...
def setup_dashboard_frame():
    ...
    booking_button = ctk.CTkButton(
        button_frame,
        text="📅 Book Appointment",
        command=go_to_booking,
        width=220,
        height=120,
        font=BUTTON_FONT,
        fg_color="#4B5563",
    ...
    notifications_button = ctk.CTkButton(
        button_frame,
        text="🔔 Notifications",
        command=go_to_notifications,
        width=220,
        height=120,
        font=BUTTON_FONT,
        fg_color="#4B5563",
...
def setup_queue_frame():
    ...
    queue_count.pack(pady=(0, 10))
    
    # Progress bar for queue position (if in queue)
    global queue_progress_label, queue_progress_bar
    queue_progress_label = ctk.CTkLabel(queue_container, text="", font=NORMAL_FONT, text_color="#4A90E2")
    queue_progress_label.pack(pady=(0, 5))
    queue_progress_bar = ctk.CTkProgressBar(queue_container, width=400, height=20)
    queue_progress_bar.pack(pady=(0, 20))
    queue_progress_bar.set(0)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if position and wait_time:
                    max_wait = max(wait_time, 30)  # At least 30 minutes scale
                    progress = min(1.0 - (wait_time / max_wait), 1.0)
                    queue_progress_bar.set(progress)
                    queue_progress_label.configure(
                        text=f"Your Position: {position} | Estimated Wait: {wait_time} minutes"
                    )
